...
export const importCSV = async (csvFilePath, type) => {
    ...
    for (const item of results) {
        try {
            if (type === "generic") {
                const { "Generic Name": name, "Unit Size": packing, MRP, "Group Name": group_name, "Drug Code": drug_code, } = item;
                if (!name)
                    continue;
                await GenericMedicine.updateOne({ name: name.trim() }, {
                    $set: {
            ...
            if (type === "branded") {
                const { "Brand Name": brand_name, "Generic Name": generic_name, Company: company, Packing: packing, "PTR (Per Unit)": price_to_retailer, MRP, } = item;
                ...
                if (!generic) {
                    console.warn(`⚠️ Skipping ${brand_name} generic '${generic_name}' not found.`);
                    continue;
                }
