...
export const addOrUpdateSharedInventory = async (
...
export const listInventory = async (
  ...
  try {
    ...
    if (role === "pharma_company") {
      ...
      branchFilter = branches.map(
        (b: any) => new mongoose.Types.ObjectId(b._id)
      );
    ...
    if (role === "pharma_branch" && branchId) {
      inventories = inventories.map((inv: any) => ({
        ...inv,
        branches: inv.branches.filter(
          (b: any) => b._id.toString() === String(branchId)
        ),
      }));
    }
...
export const removeBranchFromInventory = async (
  req: AuthenticatedRequest,
  res: Response
) => {
  try {
    ...
    const inventory = await SharedInventory.findOneAndUpdate(
      { brandedMedicineId, branchIds: branchId },
      {
