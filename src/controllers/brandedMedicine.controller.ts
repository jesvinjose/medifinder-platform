...
export const getAllBrandedMedicines = async (req: Request, res: Response) => {
  try {
...
export const getBrandedMedicineById = async (req: Request, res: Response) => {
  try {
    const medicine = await BrandedMedicine.findById(req.params.id).populate(
      "generic"
    );
    if (!medicine) {
      return res
...
export const createBrandedMedicine = async (req: Request, res: Response) => {
  try {
    const { name, generic_name, company, packing, price_to_retailer, mrp } =
      req.body;
...
export const uploadCsvAndSaveToDB = async (req: Request, res: Response) => {
  try {
    const file = req.file;
    if (!file) {
...
export const searchMedicines = async (req: Request, res: Response) => {
  try {
    ...
    const sortBy = (req.query.sortBy as string) || "mrp";
